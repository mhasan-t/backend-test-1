...
import ListUser_PostsByUserId from "../../application/use_cases/user_post/ListUser_PostsByUserId";
import DeleteManyUser_PostsByIds from "../../application/use_cases/user_post/DeleteManyUser_PostsByIds";
import { ID } from "../../domain/entities/Entity";
import DeleteManyPostsByIds from "../../application/use_cases/post/DeleteManyPostsByIds";
...
export default {
	...
	async deleteUser(request: Request, response: Response) {
		...
		// Treatment
		let res = await ListUser_PostsByUserId(serviceLocator, toDeleteUserId);

		let postsToDelete: ID[] = [];
		let pivotsToDelete: ID[] = [];
		for (let i = 0; i < res.length; i++) {
			postsToDelete.push(res[i].postId);
			pivotsToDelete.push(res[i].id as string);
		}

		await DeleteManyUser_PostsByIds(pivotsToDelete, serviceLocator);
		await DeleteManyPostsByIds(postsToDelete, serviceLocator);
